...
class HeteroGNN(LargeGNNBase):
        ...
        if "hetero_level" in self.hparams and self.hparams["hetero_level"] >= 2:
            if "recurrent" in self.hparams and not self.hparams["recurrent"]:
                self.conv = nn.ModuleList(
                    [HeteroConv(hparams) for _ in range(self.hparams["n_graph_iters"])]
                )
            else:
                self.conv = nn.ModuleList(
                    [HeteroConv(hparams)] * self.hparams["n_graph_iters"]
                )
        else:
            if "recurrent" in self.hparams and not self.hparams["recurrent"]:
                self.conv = nn.ModuleList(
                    [HomoConv(hparams) for _ in range(self.hparams["n_graph_iters"])]
                )
            else:
                self.conv = nn.ModuleList(
                    [HomoConv(hparams)] * self.hparams["n_graph_iters"]
                )
    ...
    def forward(self, x, edge_index, volume_id):
        ...
        for step in range(self.hparams["n_graph_iters"]):
            ...
            encoded_nodes, encoded_edges = checkpoint(
                self.conv[step], encoded_nodes, edge_index, encoded_edges, volume_id
            )
        ...
        if "concat_output" in self.hparams and self.hparams["concat_output"]:
            encoded_nodes, encoded_edges = checkpoint(
                self.concat_output, concat_nodes, concat_edges
            )
