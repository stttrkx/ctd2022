...
class pandaRoot_GNNTelemetry(Callback):
    """
    This callback contains standardized tests of the performance of a GNN
    """
    ...
    def get_eff_pur_metrics(self):
        ...
        )  # Make sure this can nicely be plotted!
...
# GNNBuilder Callback
class pandaRoot_GNNBuilder(Callback):
    ...
    def save_downstream(self, batch, pl_module, datatype):
        ...
        with open(
            os.path.join(self.output_dir, datatype, str(batch.event_file)),
            "wb",
        ) as pickle_file:
